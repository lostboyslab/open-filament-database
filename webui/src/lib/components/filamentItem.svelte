...
<div class="border rounded p-4 bg-white border-gray-200 text-gray-900 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-100 transition-colors relative">
  <div class="flex items-center justify-between mb-2">
    ...
    <DownloadBtn {brandName} {materialName} {filamentName} colorKey={color} />
  </div>
  {#if color.variant?.traits}
    <div class="mb-1 text-xs text-gray-700 dark:text-gray-300">
      ...
      {#each Object.entries(color.variant.traits) as [trait, value], i}
        <span>
          <svg class="w-4 h-4 text-green-600 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
