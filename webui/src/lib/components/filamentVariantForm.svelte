<script lang="ts">
  let { form, errors, message, enhance, formType: formType, brandName, materialName, filamentName } = $props();
</script>
...
<div class="max-w-md mx-auto bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-8 text-gray-900 dark:text-gray-100">
  ...
  >
    <div>
      <label for="color_name" class="block font-medium mb-1">Color name</label>
      <input
        id="color_name"
        type="text"
        name="color_name"
    ...
    <div class="mb-4">
      <label for="color_hex" class="block font-medium mb-1">Color hex</label>
    ...
    <fieldset>
      ...
      <div class="traits flex flex-wrap gap-4 mb-2">
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" id="translucent" name="translucent" bind:checked={$form.translucent} class="accent-blue-600 w-4 h-4" />
          <span>Translucent</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" id="glow" name="glow" bind:checked={$form.glow} class="accent-blue-600 w-4 h-4" />
          <span>Glow</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" id="matte" name="matte" bind:checked={$form.matte} class="accent-blue-600 w-4 h-4" />
          <span>Matte</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" id="recycled" name="recycled" bind:checked={$form.recycled} class="accent-blue-600 w-4 h-4" />
          <span>Recycled</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" id="recyclable" name="recyclable" bind:checked={$form.recyclable} class="accent-blue-600 w-4 h-4" />
          <span>Recyclable</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" id="biodegradable" name="biodegradable" bind:checked={$form.biodegradable} class="accent-blue-600 w-4 h-4" />
    ...
    <fieldset>
      ...
      <div class="sizes">
        <div class="size-item mb-4">
          <label for="filament_weight" class="block font-medium mb-1">Filament weight (g)</label>
          <input
            id="filament_weight"
            type="number"
      ...
      <div class="size-item mb-4">
        <label for="empty_spool_weight" class="block font-medium mb-1">Empty spool weight (g)</label>
        <input
      ...
      <div class="size-item mb-4">
        <label for="diameter" class="block font-medium mb-1">Diameter (mm)</label>
        <input
          id="diameter"
          type="number"
          name="diameter"
        ...
        {/if}
        <div class="size-item mb-4">
          <label for="spool_refill" class="inline-block font-medium mb-1">Spool refill</label>
          <input
            id="spool_refill"
            type="checkbox"
            name="spool_refill"
