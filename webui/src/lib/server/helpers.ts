...
export function updateBrand(
    brandData: z.infer<typeof brandSchema>
  ) {
...
export const createMaterial = (
  ...
  export function createColorFiles(formData: any) {
    ...
    if (formData.diameter_tolerance !== undefined) sizeObj.diameter_tolerance = formData.diameter_tolerance;
  ...
  export function prepareFilamentDownload(colorFolder: string, tempDownloadDir: string) {
    ...
    for (const file of ['sizes.json', 'variant.json']) {
      ...
      if (fs.existsSync(srcFile)) {
        copyFileSync(srcFile, path.join(tempDownloadDir, path.relative('src/data', path.join(colorFolder, file))));
      }
    }
  ...
  export function downloadColor(brand: string, material: string, filament: string, color: string) {
    const url = `/api/download/${encodeURIComponent(brand)}/${encodeURIComponent(material)}/${encodeURIComponent(filament)}/${encodeURIComponent(color)}`;
